...
// 트랜잭션 입력 구조 정의
export class TxIn {
  txOutId?: string; // 이전 트랜잭션의 아이디 (해시값)
...
// 200 account utxo (미사용 객체를 모아두는 공간이다.)
...
export class UnspentTxOut {
  txOutId: string; // hash 값 해당 utxo가 포함된 트랜잭션 hash 값
  txOutIndex: number; // 해당 UTXO가 포함된 트랜잭션의 출력 인덱스
  account: string; // utxo 소유 계정
  amount: number; // 잔액
}
// 트랜잭션 데이터 타입 정의 문자또는 row가 될수도있다.
// 전송시 문자열로
export type TrransactionData = string | TransactionRow[];
// 사용되지않은 않은 UTXO 출력
export type UnspentTxOutPool = UnspentTxOut[];

// 트랜잭션 pool의 타입 정의
// 트랜잭션을 발생시키면 바로 처리되는게 아니라
// 트랜잭션 pool라는 공간에 처리되지않은 트랜잭션이 대기 상태로 쌓이고
// 블록이 생성될떄 트랜잭션 pool 에 있는 대기 상태의 트랜잭션을 처리하고 블록에 기록한다.
